...
export default async (req) => {
  ...
  const items = (body.items || []).map(i => { const p = PRODUCTS[i.slug]; if (!p) return null; return { slug: i.slug, name: p.name, qty: Math.max(1, Math.min(10, Number(i.qty) || 1)), unit: i.sub ? p.sub : p.price, sub: !!i.sub }; }).filter(Boolean);
  ...
  const order = { id: orderId, t: Date.now(), items, subtotal, discount, code, shipping, total, email: body.email, name: body.name, address: body.address, pay: body.pay, vid: body.vid, sid: body.sid, utm: body.utm || {}, status: 'pending', upsell: null };
