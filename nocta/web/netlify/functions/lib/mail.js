...
export const tpl = {
  ...
  orderConfirm(o, recompensa) {
    ...
    // Un pedido de prueba manda el correo de verdad —es la única forma de comprobar que el correo
    // llega y se ve bien— pero lo dice en el asunto y arriba del todo. Un email que parece una
    // compra real y no lo es es peor que no mandarlo.
    const pru = o.demo === true;
    const aviso = pru ? `<p style="margin:0 0 18px;padding:10px 14px;border-radius:8px;background:#FDF0E6;color:#8A4B1E;font-size:14px"><b>Pedido de prueba.</b> No se ha cobrado nada: la tienda todavía no tiene la pasarela de pago conectada.</p>` : '';
    return { subject: `${pru ? '[PRUEBA] ' : ''}Pedido ${o.id} confirmado · sale en 24-48 h`, text: `${pru ? '[PRUEBA · no se ha cobrado nada] ' : ''}Gracias. Tu pedido ${o.id} (${eur(o.total)}) sale del almacén en 24-48 h.${r ? ` Tu código para la próxima: ${r.code} (−${Number(r.value)} %).` : ''}`, html: layout({ title: 'Pedido confirmado', preheader: pru ? 'Pedido de prueba: no se ha cobrado nada.' : (r ? `En marcha. Y dentro, tu −${Number(r.value)} % para la próxima.` : `Tu pedido ${o.id} está en marcha.`), body: aviso + h.lab('Pedido ' + o.id) + h.title('Gracias. Tu pedido está en marcha.') + h.p('Sale de nuestro almacén en España en 24-48 h laborables. Te enviamos el seguimiento en cuanto salga.') + h.items(o.items || []) + `<p style="margin:12px 0 0;font-size:15px;text-align:right;color:#14213D">${o.shipping ? 'Envío ' + eur(o.shipping) + ' · ' : 'Envío gratis · '}<b>Total ${eur(o.total)}</b></p>` + bloque + h.hr() + h.lab('Mientras llega') + h.p('Lee la guía: piel seca, 20 segundos de presión y 6-8 horas. Así el parche sale lleno la primera mañana.') + h.btn('Cómo usar los parches', site + '/como-usar.html') }) };
  },
