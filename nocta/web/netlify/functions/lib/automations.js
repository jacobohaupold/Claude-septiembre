  ...
  if (ab) {
    ...
    try {
      ...
      for (const c of carts || []) {
        ...
        if (ab.whatsapp !== false) { const l = await db.one('leads', 'email=eq.' + esc(c.email) + '&wa_optin=eq.true&phone=not.is.null'); if (l) await waSend({ to: l.phone, kind: 'abandoned_cart', text: fill(ab.wa_text || 'Hola {nombre} 🌙 Te hemos guardado la cesta en NOCTA. Con el código *{code}* tienes un {pct} %: {url}', { nombre: l.name ? String(l.name).split(' ')[0] : '', code, pct, url: SITE() + '/checkout.html?code=' + code }), meta: { vid: c.vid } }); }
  ...
  if (w) {
    try {
      ...
      for (const c of cs || []) { if ((c.tags || []).includes('winback_sent')) continue; const sub = await db.one('subscriptions', 'email=eq.' + esc(c.email) + '&status=in.(active,trialing,cancelling)'); if (sub) continue;
        ...
        if (w.whatsapp) { const l = await db.one('leads', 'email=eq.' + esc(c.email) + '&wa_optin=eq.true&phone=not.is.null'); if (l) await waSend({ to: l.phone, kind: 'winback', text: fill(w.wa_text || 'Hola {nombre} 🌙 Los filamentos vuelven cada pocas semanas. Para repetir, *{code}* te descuenta un {pct} %: {url}', { nombre: c.name ? String(c.name).split(' ')[0] : '', code: w.code || 'VUELVE15', pct: w.pct || 15, url: SITE() + '/catalogo.html?code=' + (w.code || 'VUELVE15') }) }); }
