...
/* «Mensualidad» (antes «suscripción»): el % sale del CRM (content.pricing.sub_pct) y manda el precio
   real del producto si alguien le ha puesto un sub a mano en la ficha. Un solo sitio para toda la web. */
window.nSubPct=p=>{if(p&&!p.plan&&p.sub&&p.price&&p.sub<p.price)return Math.round((1-p.sub/p.price)*100);
  const c=window.NOCTA_CFG&&window.NOCTA_CFG.pricing&&window.NOCTA_CFG.pricing.sub_pct;return c==null?20:Math.round(+c);};
window.nSubName='Mensualidad';
/* El descuento es lo único de la web pintado en terracota: así se encuentra sin buscarlo.
   Por producto se enciende como mucho UNA píldora sólida (el descuento más grande que tiene) y,
   donde hay sitio, la segunda en versión suave. Dos píldoras a tope se estorban entre ellas. */
window.nDtoPct=p=>(p&&p.compare&&p.compare>p.price)?Math.round((1-p.price/p.compare)*100):0;
/* En la rejilla se enseña UN solo número y nada más: el descuento. Sin «cada mes» —que sea una
   mensualidad se cuenta dentro de la ficha, no en la tarjeta— y sin dos porcentajes compitiendo.
   Si la chapa de la esquina ya canta el ahorro (un pack con precio tachado, o un «AHORRA 12 €»),
   la píldora sobra: repetir el mismo descuento dos veces resta en vez de sumar. */
window.nDto=(p,{s=false}={})=>{
  if(!p||p.plan)return'';
  if(nDtoPct(p)>=5||(p.badge&&/ahorra/i.test(p.badge)))return'';
  const m=(p.sub&&p.sub<p.price)?nSubPct(p):0;
  return m>=5?`<span class="n-dtos"><span class="n-dto${s?' n-dto--s':''}">−${m} %</span></span>`:'';};
/* Nota personal de cada producto: por qué es para ti, o qué se te va a olvidar. */
window.nPers=p=>(p&&p.pers&&p.pers.nota)?p.pers:null;
window.nPersRec=p=>!!(p&&p.pers&&/^Lo más/.test(p.pers.tag||''));
...
const BADGE=p=>p.badge?(/ahorra/i.test(p.badge)
  ?`<span class="n-pc__new n-pc__new--dto n-dto">${p.badge.toLowerCase()}</span>`
  :`<span class="n-pc__new${/NUEVO/i.test(p.badge)?'':' n-pc__new--v'}">${/NUEVO/i.test(p.badge)?'nuevo':p.badge.toLowerCase()}</span>`):'';
const MINIB=p=>p.compare?`<span class="n-pm__b n-pm__b--dto n-dto">−${nDtoPct(p)} %</span>`:(p.badge?`<span class="n-pm__b${/ahorra/i.test(p.badge)?' n-pm__b--dto n-dto':' n-pm__b--v'}">${p.badge.toLowerCase()}</span>`:'');
window.nCardR=(p,o={})=>{const ph=NOCTA_PHOTOS[p.slug]||{};const lim=ph.limpio;const src=lim||ph.main||nImg(p);const cut=lim?true:(!ph.main&&nIsCut(src));const w=CATW[p.slug]||nCat(p).toLowerCase();
  return`<article class="n-pc${lim?' n-pc--float':''} n-reveal" data-slug="${p.slug}"><a class="n-pc__w" href="/producto.html?p=${p.slug}">${lim?'':BADGE(p)}<img src="${src}" class="${cut?'is-cut':''}" width="960" height="960" alt="${p.name}" loading="${o.eager?'eager':'lazy'}"${o.eager?' fetchpriority="high"':''} decoding="async">${ph.alt?`<img src="${ph.alt}" class="alt" width="960" height="960" alt="" loading="lazy" decoding="async">`:''}</a><div class="n-pc__i">${lim?BADGE(p):''}<p class="n-pc__r">${nStarsHtml(p)}<span>(${p.reviews})</span></p><div class="n-pc__t"><b>${p.name}</b><span class="n-num">${eur(p.price)}${nPlanSuf(p)}</span></div><p class="n-pc__s">${p.short}${p.compare?` · <s>${eur(p.compare)}</s>`:''}</p>${p.pers?`<p class="n-pc__pers${nPersRec(p)?' is-rec':''}">${p.pers.tag}</p>`:''}${nDto(p)}<button class="n-btn" data-buy="${p.slug}" data-src="${o.src||'card'}">${p.plan?'Empezar':'Comprar'} ${w}<span class="n-pc__bp"> · ${eur(p.price)}${p.plan?'/'+p.plan.per:''}</span></button>${WALLET!=='plain'?nPay(p.slug,{src:o.src||'card'}):''}</div></article>`;};
/* tarjeta compacta (portada, «lo más vendido»): foto 1:1, nombre, precio, Comprar y pago rápido, todo a la vista */
window.nCardM=(p,o={})=>{const ph=NOCTA_PHOTOS[p.slug]||{};const lim=ph.limpio;const src=lim||ph.main||nImg(p);const cut=lim?true:(!ph.main&&nIsCut(src));const w=CATW[p.slug]||nCat(p).toLowerCase();
  return`<article class="n-pm${lim?' n-pm--float':''} n-reveal" data-slug="${p.slug}"><a class="n-pm__w" href="/producto.html?p=${p.slug}" aria-label="${p.name}">${lim?'':MINIB(p)}<img src="${src}" class="${cut?'is-cut':''}" width="720" height="720" alt="" loading="${o.eager?'eager':'lazy'}"${o.eager?' fetchpriority="high"':''} decoding="async"></a><div class="n-pm__i"><span class="n-pm__mk">${lim?MINIB(p):''}${nDto(p,{s:true})}</span><b>${p.name}</b><span class="n-num">${eur(p.price)}${nPlanSuf(p)}${p.compare?` <s>${eur(p.compare)}</s>`:''}</span></div><button class="n-btn n-btn--fill n-pm__buy" data-buy="${p.slug}" data-src="${o.src||'mini'}">${p.plan?'Empezar':'Comprar'}</button>${nPay(p.slug,{src:o.src||'mini'})}</article>`;};
...
/* ---------- Plan personalizado (mensual): zonas 1-3 + skincare opcional; precio y opt compartidos con carrito, checkout y servidor ---------- */
const PZ=[['Nariz','parches-nariz'],['Frente','parches-frente'],['Barbilla','parches-barbilla'],['Granos','parches-granos']],PS=[['Exfoliante','exfoliante-salicilico'],['Sérum','serum-niacinamida'],['Tónico','tonico-hialuronico']];
window.NOCTA_PLAN={zones:PZ,skin:PS,defaultOpt:'Nariz + Frente · Exfoliante'};
window.nPlanCalc=(p,opt)=>{if(!p||!p.plan)return null;const b=p.plan.builder;
  if(!b){const z=PZ.find(x=>x[0]===String(opt||'').trim())||PZ[0];const base=byslug(z[1]);return{price:p.price,zones:[z[0]],slugs:[z[1]],skin:null,retail:base?base.price+SHIP.base:p.compare||p.price,opt:z[0]};}
  const parts=String(opt||window.NOCTA_PLAN.defaultOpt).split(' · ');const zones=parts[0].split(' + ').map(x=>x.trim()).filter(x=>PZ.some(z=>z[0]===x));if(!zones.length)zones.push('Nariz');
  const sk=PS.find(x=>x[0]===(parts[1]||'').trim())||null;const slugs=[...zones.map(z=>PZ.find(x=>x[0]===z)[1]),...(sk?[sk[1]]:[])];
  const price=(b.patch[Math.min(b.patch.length-1,zones.length)]??b.patch[b.patch.length-1])+(sk?b.skincare:0);let retail=slugs.reduce((a,sl)=>{const q=byslug(sl);return a+(q?q.price:0);},0);if(retail<SHIP.freeFrom)retail+=SHIP.base;
  return{price,zones,skin:sk?sk[0]:null,skinSlug:sk?sk[1]:null,slugs,retail:+retail.toFixed(2),save:+(retail-price).toFixed(2),opt:zones.join(' + ')+' · '+(sk?sk[0]:'Sin skincare'),n:zones.length};};
window.nPlanBuilder=(el,p,o={})=>{const isW=p.plan.interval==='week';let st=nPlanCalc(p,o.opt);
  /* Cada zona y cada skincare llevan su propio aviso: por qué ese y qué pasa si lo dejas fuera.
     Se redibuja con la selección, así que lo que lees siempre habla de lo que llevas marcado. */
  const aviso=sl=>{const q=byslug(sl),pe=q&&q.pers;return pe?`<li class="${/^Lo más/.test(pe.tag)?'is-rec':''}"><b>${q.name} · ${pe.tag}</b><span>${pe.nota}</span></li>`:'';};
  const tips=()=>{const ls=[...st.zones.map(z=>(PZ.find(x=>x[0]===z)||[])[1]),...(st.skinSlug?[st.skinSlug]:[])].filter(Boolean).map(aviso).join('');
    return ls?`<ul class="n-plb__tips">${ls}</ul>`:'';};
  const pill=([z,sl])=>{const q=byslug(sl);const rec=q&&q.pers&&/^Lo más/.test(q.pers.tag||'');
    return `<button type="button" data-z="${z}" class="${st.zones.includes(z)?'is-on':''}${rec?' is-rec':''}" aria-pressed="${st.zones.includes(z)}"${q&&q.pers?` title="${q.pers.tag}"`:''}>${z}${rec?'<i aria-hidden="true">★</i>':''}</button>`;};
  const draw=()=>{el.innerHTML=`<div class="n-plb"><span class="n-lab">${isW?'Zona de esta semana':'Zonas de parches · elige las que quieras'}</span><div class="n-plb__pills" role="group">${PZ.map(pill).join('')}</div>${isW?'':`<span class="n-lab">Skincare para poros cada mes</span><div class="n-plb__pills" role="radiogroup">${PS.map(([k])=>`<button type="button" data-s="${k}" class="${st.skin===k?'is-on':''}" aria-pressed="${st.skin===k}">${k}</button>`).join('')}<button type="button" data-s="" class="${st.skin?'':'is-on'}" aria-pressed="${!st.skin}">Sin skincare</button></div>`}${tips()}</div>`;
    el.querySelectorAll('[data-z]').forEach(b=>b.onclick=()=>{const z=b.dataset.z;let zs=st.zones.slice();if(isW)zs=[z];else if(zs.includes(z)){if(zs.length>1)zs=zs.filter(x=>x!==z);}else{zs.push(z);const mx=(p.plan.builder&&p.plan.builder.patch?p.plan.builder.patch.length-1:4);if(zs.length>mx)zs.shift();}st=nPlanCalc(p,zs.join(' + ')+(isW?'':' · '+(st.skin||'Sin skincare')));draw();o.onChange&&o.onChange(st);T.send('plan_zone',{plan:p.slug,opt:st.opt});});
    el.querySelectorAll('[data-s]').forEach(b=>b.onclick=()=>{st=nPlanCalc(p,st.zones.join(' + ')+' · '+(b.dataset.s||'Sin skincare'));draw();o.onChange&&o.onChange(st);T.send('plan_skin',{plan:p.slug,opt:st.opt});});};
  draw();return{get state(){return st;}};};

/* precio unitario con descuento por cantidad (CRM → content.pricing.multi) */
window.nUnit=(p,qty=1,sub=false)=>{const base=sub?p.sub:p.price;const m=(window.NOCTA_CFG&&window.NOCTA_CFG.pricing&&window.NOCTA_CFG.pricing.multi)||{};const pct=qty>=3?+(m[3]||0):qty>=2?+(m[2]||0):0;return +(base*(1-pct/100)).toFixed(2);};
/* ---------- Compra exprés ---------- */
...
async function nBuyRun(items,{via='buy_now',src='btn'}={}){
  ...
  const value=items.reduce((a,i)=>{const p=byslug(i.slug);return a+(p.plan?nPlanCalc(p,i.opt).price:nUnit(p,i.qty,i.sub))*i.qty;},0);
...
const C={
  ...
  totals(){const items=this.get();let sub=0,n=0,plan=false;items.forEach(i=>{const p=byslug(i.slug);if(!p)return;if(p.plan)plan=true;sub+=(p.plan?nPlanCalc(p,i.opt).price:nUnit(p,i.qty,i.sub))*i.qty;n+=i.qty;});if(plan)n=Math.max(n,2);
  ...
  render(){const t=this.totals();const cnt=$('#cartcount');if(cnt){cnt.textContent=t.n;if(t.n)cnt.removeAttribute('data-zero');else cnt.setAttribute('data-zero','');const ob=$('#opencart');if(ob)ob.setAttribute('aria-label',`Abrir carrito, ${t.n} artículo${t.n===1?'':'s'}`);}
    ...
    const up=$('#cartupsell');if(up){const slugs=t.items.map(i=>i.slug);let s=null;
      const rules=(window.NOCTA_CFG&&Array.isArray(window.NOCTA_CFG.cart_upsell)&&window.NOCTA_CFG.cart_upsell.length)?window.NOCTA_CFG.cart_upsell:[{if:'parches-nariz',then:'exfoliante-salicilico',unless:['duo-poros']},{then:'parches-granos',unless:['kit-cara-completa']},{then:'serum-niacinamida'}];
      for(const r of rules){if(r.if&&!slugs.includes(r.if))continue;if(!r.then||slugs.includes(r.then))continue;if((r.unless||[]).some(u=>slugs.includes(u)))continue;if(!byslug(r.then))continue;s=r.then;break;}
...
function layout(){
  ...
  const CFGN=window.NOCTA_CFG||{};const NAV=(CFGN.nav&&Array.isArray(CFGN.nav.items)&&CFGN.nav.items.length)?CFGN.nav.items.map(r=>[r.href,r.label]):[['/#productos','Parches'],['/#skincare','Skincare'],['/#packs','Packs'],['/planes.html','Planes'],['/catalogo.html','Catálogo'],['/no-son-puntos-negros.html','No son puntos negros'],['/quiz.html','Test de piel'],['/garantia.html','Garantía']];
...
<header class="n-top"><div class="n-wrap n-top__in"><button class="n-icon" id="opennav" aria-label="Abrir menú" aria-expanded="false" aria-controls="n-nav">${ico('menu',22)}</button><a class="n-logo" href="/" aria-label="NOCTA, inicio">${ico('moon',18)}nocta</a><nav class="n-main" aria-label="Principal">${NAV.map(r=>`<a href="${r[0]}">${r[1]}</a>`).join('')}</nav><button class="n-cart" id="opencart" aria-label="Abrir carrito, 0 artículos">${ico('bag',22)}<b id="cartcount" data-zero>0</b></button></div></header>`;
  $$('.n-main a').forEach(a=>{if(a.getAttribute('href').split('#')[0]===path)a.setAttribute('aria-current','page');});
  /* aviso configurable desde el CRM (content.announce) */
  if(CFGN.announce&&CFGN.announce.enabled&&CFGN.announce.text&&h){const an=document.createElement('div');an.className='n-announce';an.innerHTML=CFGN.announce.href?`<a href="${CFGN.announce.href}">${CFGN.announce.text}</a>`:`<span>${CFGN.announce.text}</span>`;h.after(an);}
  /* textos editables desde el CRM (data-cms="clave") */
  if(CFGN.cms){$$('[data-cms]').forEach(el=>{const v=CFGN.cms[el.dataset.cms];if(typeof v==='string'&&v.trim())el.textContent=v;});}
  ...
  const rows=(CFGN.nav&&Array.isArray(CFGN.nav.menu)&&CFGN.nav.menu.length)?CFGN.nav.menu.map(r=>[r.href,r.label,r.note||'']):[['/#productos','Parches','5 formatos'],['/#skincare','Skincare','5 fórmulas'],['/#packs','Packs','ahorra hasta 24 €'],['/planes.html','Planes','desde 14,90 €/sem'],['/catalogo.html','Catálogo','13 productos'],['/quiz.html','Test de piel','4 preguntas'],['/no-son-puntos-negros.html','No son puntos negros','3 min'],['/como-usar.html','Cómo usar',''],['/garantia.html','Garantía','60 días']];
  ...
  const f=$('#site-footer');if(f)f.innerHTML=isCo?`<footer class="n-foot"><div class="n-wrap"><a href="/legal.html#aviso">Aviso legal</a><a href="/legal.html#privacidad">Privacidad</a><a href="/legal.html#cookies">Cookies</a><a href="/contacto.html">Contacto</a><a href="/garantia.html">Garantía 60 días</a></div></footer>`
    :`<footer class="n-foot"><div class="n-wrap"><div class="n-foot__brand"><h3 class="n-lab">NOCTA</h3><p>${(CFGN.footer&&CFGN.footer.text)||'Parches de hidrocoloide coreanos y skincare para poros, diseñados en España. Poros limpios mientras duermes.'}</p><p class="n-xs" style="margin-top:8px">NOCTA Skin S.L. (en constitución) · Persona Responsable en la UE · Cosméticos notificados en el CPNP.</p></div><nav aria-label="Tienda"><h3 class="n-lab">Tienda</h3><a href="/#productos">Parches</a><a href="/#skincare">Skincare</a><a href="/#packs">Packs y ahorro</a><a href="/catalogo.html">Catálogo completo</a><a href="/producto.html?p=parches-nariz">Parches de nariz</a></nav><nav aria-label="Ayuda"><h3 class="n-lab">Ayuda</h3><a href="/como-usar.html">Cómo usar</a><a href="/garantia.html">Garantía 60 días</a><a href="/envios-devoluciones.html">Envíos y devoluciones</a><a href="/contacto.html">Contacto</a><a href="/ciencia.html">Nuestra ciencia</a><a href="/sobre.html">Sobre NOCTA</a></nav><nav aria-label="Legal"><h3 class="n-lab">Legal</h3><a href="/legal.html#aviso">Aviso legal</a><a href="/legal.html#privacidad">Privacidad</a><a href="/legal.html#cookies">Cookies</a><a href="/legal.html#envios">Condiciones de envío</a></nav><p class="n-foot__legal n-xs">${(CFGN.footer&&CFGN.footer.legal)||'NOCTA · hola@nocta.es · Enviamos desde España · © 2026'}</p><div class="n-foot__mark" aria-hidden="true">${ico('moon',18)}</div></div></footer>`;
  /* captación (popup v8: centrado, fondo desenfocado, copy personal y estado de éxito) */
  const CFG=window.NOCTA_CFG||{},PC=Object.assign({enabled:true,code:'HOLA10',pct:10,title:'En tu primera caja.',text:'Dinos tu nombre y te enviamos el código ahora mismo. Vale para todo: parches, packs y planes.',wa_text:'Avísame por WhatsApp del código y del envío. Sin spam.',require_phone:true,img:'/assets/img/hero-poster.webp'},CFG.popup||{});
  /* sale nada más entrar en la PRIMERA página de la visita (portada, producto o cualquier otra: también si llega desde un anuncio);
     en las siguientes páginas de esa visita ya no. Una vez por dispositivo; si se cierra sin registrarse vuelve a los 3 días; ?popup=1 lo fuerza. */
  const PK='n_popup8',pv=localStorage.getItem(PK),force=new URL(location.href).searchParams.get('popup')==='1';
  const seen=pv==='done'||(pv&&Date.now()-Number(pv)<3*864e5);let landing=true;try{landing=!sessionStorage.getItem('n_pop_s');}catch(e){}
  if(PC.enabled!==false&&(force||(!seen&&landing))&&!isCo&&!path.includes('admin')){let taps=1;
    const title=t=>String(t).replace('{pct}',PC.pct),reviews=P.reduce((a,p)=>a+(p.reviews||0),0);
    const pop=document.createElement('div');pop.className='n-mail';pop.setAttribute('role','dialog');pop.setAttribute('aria-modal','true');pop.setAttribute('aria-label',title(PC.title));pop.innerHTML=`<div class="box"><button class="n-icon x" id="popx" aria-label="Cerrar">${ico('x',18)}</button><div class="n-mail__in"><p class="n-mail__over">${ico('moon',11)} Primera caja</p><p class="n-mail__big"><span class="n-num">−${PC.pct}</span><small>%</small></p><h3 id="poptitle">${title(PC.title)}</h3><p class="n-mail__lead">${PC.text}</p><form id="popform" novalidate><label class="n-vh" for="popname">Nombre</label><input id="popname" name="name" required placeholder="Nombre" autocomplete="given-name" enterkeyhint="next"><label class="n-vh" for="popmail">Email</label><input id="popmail" name="email" type="email" required placeholder="Email" autocomplete="email" enterkeyhint="next"><label class="n-vh" for="popphone">Móvil</label><input id="popphone" name="phone" type="tel" inputmode="tel" ${PC.require_phone?'required ':''}placeholder="Móvil · para el WhatsApp" autocomplete="tel" enterkeyhint="done"><label class="n-mail__chk"><input type="checkbox" id="popwa" checked><span>${PC.wa_text}</span></label><button class="n-btn n-btn--fill n-btn--wide" id="popgo">Enviarme mi −${PC.pct}\u00a0%</button></form><p class="n-mail__fine"><span class="n-stars" aria-hidden="true">★★★★★</span> 4,8 · ${reviews.toLocaleString('es-ES')} opiniones · <a href="/legal.html#privacidad">Privacidad</a></p></div><div class="n-mail__ok"><div class="tick">${ico('check',24)}</div><h3 id="poptitle2">Listo.</h3><p class="n-mail__lead" id="popmsg"></p><span class="code" id="popcode"></span><p class="n-mail__fine">Se aplica solo al pagar. También te lo hemos enviado por email.</p><button class="n-btn n-btn--fill n-btn--wide" id="popdone">Elegir mis parches</button></div></div>`;document.body.appendChild(pop);
    const openP=()=>{if(localStorage.getItem(PK)==='done'||document.body.classList.contains('n-locked')||pop.classList.contains('is-open'))return;try{sessionStorage.setItem('n_pop_s','1');}catch(e){}pop.classList.remove('is-closing');raf2(()=>pop.classList.add('is-open'));T.send('popup_view');setTimeout(()=>{const n=$('#popname');if(n&&DESK())n.focus({preventScroll:true});},650);};
    const wait=force?600:Math.min(3000,PC.delay_s?PC.delay_s*1000:1200);setTimeout(openP,wait);document.addEventListener('mouseleave',e=>{if(e.clientY<10)openP();});
    ...
    $('#popname').addEventListener('input',e=>{const n=e.target.value.trim().split(' ')[0];$('#poptitle').textContent=n?`${n.charAt(0).toUpperCase()+n.slice(1)}, en tu primera caja.`:title(PC.title);$('#popgo').textContent=n?`Enviarme mi −${PC.pct}\u00a0%, ${n.charAt(0).toUpperCase()+n.slice(1)}`:`Enviarme mi −${PC.pct}\u00a0%`;});
