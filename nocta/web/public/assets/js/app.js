...
window.nCardR=(p,o={})=>{const ph=NOCTA_PHOTOS[p.slug]||{};const src=ph.main||nImg(p);const cut=!ph.main&&nIsCut(src);const w=CATW[p.slug]||nCat(p).toLowerCase();
  return`<article class="n-pc n-reveal" data-slug="${p.slug}"><a class="n-pc__w" href="/producto.html?p=${p.slug}"><span class="n-pc__cat" aria-hidden="true">${w}</span>${p.badge&&/NUEVO/i.test(p.badge)?'<span class="n-pc__new">nuevo</span>':''}<img src="${src}" class="${cut?'is-cut':''}" width="960" height="960" alt="${p.name}" loading="lazy" decoding="async">${ph.alt?`<img src="${ph.alt}" class="alt" width="960" height="960" alt="" loading="lazy" decoding="async">`:''}</a><div class="n-pc__i"><p class="n-pc__r">${nStarsHtml(p)}<span>(${p.reviews})</span></p><div class="n-pc__t"><b>${p.name}</b><span class="n-num">${eur(p.price)}</span></div><p class="n-pc__s">${p.short}${p.compare?` · <s>${eur(p.compare)}</s>`:''}</p><button class="n-btn" data-buy="${p.slug}" data-src="${o.src||'card'}">Comprar ${w} · ${eur(p.price)}</button>${WALLET!=='plain'?nPay(p.slug,{src:o.src||'card'}):''}</div></article>`;};
