...
<header class="n-top"><div class="n-wrap n-top__in"><button class="n-icon" id="opennav" aria-label="Abrir menú" aria-expanded="false" aria-controls="n-nav">${ico('menu',22)}</button><a class="n-logo" href="/" aria-label="NOCTA, inicio">${ico('moon',18)}nocta</a><nav class="n-main" aria-label="Principal"><a href="/#productos">Parches</a><a href="/#skincare">Skincare</a><a href="/#packs">Packs</a><a href="/planes.html">Planes</a><a href="/catalogo.html">Catálogo</a><a href="/no-son-puntos-negros.html">No son puntos negros</a><a href="/quiz.html">Test de piel</a><a href="/garantia.html">Garantía</a></nav><button class="n-cart" id="opencart" aria-label="Abrir carrito, 0 artículos">${ico('bag',22)}<b id="cartcount" data-zero>0</b></button></div></header>`;
  ...
  /* captación (popup v7: nombre + email + móvil, consentimiento WhatsApp, −10 % configurable desde el CRM) */
  const CFG=window.NOCTA_CFG||{},PC=Object.assign({enabled:true,code:'HOLA10',pct:10,title:'Tu primera caja con −10 %',text:'Déjanos tu nombre, email y móvil y te enviamos el código al momento. Y la guía «Por qué tus puntos negros vuelven».',wa_text:'Quiero recibir novedades y ofertas por email y WhatsApp',require_phone:true},CFG.popup||{});
  if(PC.enabled!==false&&!localStorage.getItem('n_popup')&&!isCo&&!path.includes('admin')){let taps=0;document.addEventListener('pointerdown',()=>taps++,{passive:true});
    const pop=document.createElement('div');pop.className='n-mail';pop.setAttribute('role','dialog');pop.setAttribute('aria-label',PC.title);pop.innerHTML=`<div class="box"><button class="n-icon x" id="popx" aria-label="Cerrar">${ico('x',20)}</button><span class="n-lab">Bienvenida a NOCTA</span><h3 class="n-d3" style="margin:6px 0 4px">${PC.title}</h3><p class="n-sm n-muted">${PC.text}</p><form id="popform" novalidate><div class="n-mail__row"><div><label class="n-vh" for="popname">Nombre</label><input id="popname" name="name" required placeholder="Tu nombre" autocomplete="given-name" enterkeyhint="next"></div><div><label class="n-vh" for="popphone">Móvil</label><input id="popphone" name="phone" type="tel" inputmode="tel" ${PC.require_phone?'required ':''}placeholder="Móvil" autocomplete="tel" enterkeyhint="next"></div></div><label class="n-vh" for="popmail">Email</label><input id="popmail" name="email" type="email" required placeholder="tu@email.com" autocomplete="email" enterkeyhint="done"><label class="n-mail__chk"><input type="checkbox" id="popwa" checked><span>${PC.wa_text}</span></label><button class="n-btn n-btn--wide" id="popgo">Quiero mi −${PC.pct} %</button></form><p class="n-xs n-muted" style="margin-top:8px">Sin spam. Baja cuando quieras. <a href="/legal.html#privacidad">Privacidad</a></p></div>`;document.body.appendChild(pop);
    ...
    setTimeout(show,PC.delay_s?PC.delay_s*1000:(path.includes('producto')?20000:12000));document.addEventListener('mouseleave',e=>{if(e.clientY<10)show();});
    ...
    $('#popform').onsubmit=e=>{e.preventDefault();const f=e.target;if(!f.checkValidity()){f.reportValidity();return;}const data={name:$('#popname').value.trim(),email:$('#popmail').value.trim(),phone:$('#popphone').value.trim(),wa:$('#popwa').checked,src:'popup',utm:JSON.parse(sessionStorage.getItem('n_utm')||'{}'),vid:T.sid};
      ...
      const done=j=>{const code=(j&&j.code)||PC.code,pct=(j&&j.pct)||PC.pct;sessionStorage.setItem('n_disc',JSON.stringify({code,type:'pct',value:pct}));localStorage.setItem('n_lead',JSON.stringify({email:data.email,name:data.name,phone:data.phone}));hide();toast(`Código ${code} aplicado (−${pct} %)`);T.send('lead',{src:'popup',wa:data.wa});C.render();C.sync(true);};
