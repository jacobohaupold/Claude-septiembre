...
/* opiniones añadidas desde el CRM (antes de que la ficha las lea) */
(()=>{const c=window.NOCTA_CFG;if(c&&c.reviews&&Array.isArray(c.reviews.extra)&&window.NOCTA_REVIEWS)c.reviews.extra.forEach(r=>{if(!r.slug)return;(NOCTA_REVIEWS[r.slug]=NOCTA_REVIEWS[r.slug]||[]).unshift([r.name||'Cliente',r.city||'',+r.stars||5,r.text||'']);});})();
...
window.nCardM=(p,o={})=>{const ph=NOCTA_PHOTOS[p.slug]||{};const src=ph.main||nImg(p);const cut=!ph.main&&nIsCut(src);const w=CATW[p.slug]||nCat(p).toLowerCase();
...
window.nPlanCalc=(p,opt)=>{if(!p||!p.plan)return null;const b=p.plan.builder;
  ...
  const price=b.patch[Math.min(3,zones.length)]+(sk?b.skincare:0);let retail=slugs.reduce((a,sl)=>{const q=byslug(sl);return a+(q?q.price:0);},0);if(retail<SHIP.freeFrom)retail+=SHIP.base;
...
window.nPlanBuilder=(el,p,o={})=>{const isW=p.plan.interval==='week';let st=nPlanCalc(p,o.opt);
  const draw=()=>{el.innerHTML=`<div class="n-plb"><span class="n-lab">${isW?'Zona de esta semana':'Zonas de parches · elige 1, 2 o 3'}</span><div class="n-plb__pills" role="group">${PZ.map(([z])=>`<button type="button" data-z="${z}" class="${st.zones.includes(z)?'is-on':''}" aria-pressed="${st.zones.includes(z)}">${z}</button>`).join('')}</div>${isW?'':`<span class="n-lab">Skincare para poros cada mes</span><div class="n-plb__pills" role="radiogroup">${PS.map(([k])=>`<button type="button" data-s="${k}" class="${st.skin===k?'is-on':''}" aria-pressed="${st.skin===k}">${k}</button>`).join('')}<button type="button" data-s="" class="${st.skin?'':'is-on'}" aria-pressed="${!st.skin}">Sin skincare</button></div>`}</div>`;
    el.querySelectorAll('[data-z]').forEach(b=>b.onclick=()=>{const z=b.dataset.z;let zs=st.zones.slice();if(isW)zs=[z];else if(zs.includes(z)){if(zs.length>1)zs=zs.filter(x=>x!==z);}else{zs.push(z);if(zs.length>3)zs.shift();}st=nPlanCalc(p,zs.join(' + ')+(isW?'':' · '+(st.skin||'Sin skincare')));draw();o.onChange&&o.onChange(st);T.send('plan_zone',{plan:p.slug,opt:st.opt});});
