...
const BADGE=p=>p.badge?`<span class="n-pc__new${/NUEVO/i.test(p.badge)?'':' n-pc__new--v'}">${/NUEVO/i.test(p.badge)?'nuevo':p.badge.toLowerCase()}</span>`:'';
const MINIB=p=>p.compare?`<span class="n-pm__b">−${Math.round((1-p.price/p.compare)*100)} %</span>`:(p.badge?`<span class="n-pm__b n-pm__b--v">${p.badge.toLowerCase()}</span>`:'');
