...
async function nBuyRun(items,{via='buy_now',src='btn'}={}){
  ...
  const value=items.reduce((a,i)=>{const p=byslug(i.slug);return a+(p.plan?nPlanCalc(p,i.opt).price:(i.sub?p.sub:p.price))*i.qty;},0);
...
const C={
  ...
  totals(){const items=this.get();let sub=0,n=0,plan=false;items.forEach(i=>{const p=byslug(i.slug);if(!p)return;if(p.plan)plan=true;sub+=(p.plan?nPlanCalc(p,i.opt).price:(i.sub?p.sub:p.price))*i.qty;n+=i.qty;});if(plan)n=Math.max(n,2);
