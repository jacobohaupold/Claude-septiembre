...
window.nDto=(p,{s=false,max=2,comp=true,suave=false}={})=>{const l=[],c=nDtoPct(p),m=(!p.plan&&p.sub&&p.sub<p.price)?nSubPct(p):0;
  if(c>=5&&comp)l.push(`−${c} %`);
  if(m>=5)l.push(`−${m} % cada mes`);
  if(!l.length)return'';
  return`<span class="n-dtos">${l.slice(0,max).map((t,i)=>`<span class="n-dto${(i||suave)?' n-dto--2':''}${s?' n-dto--s':''}">${t}</span>`).join('')}</span>`;};
...
const CHAPA_DTO=p=>!!(p.badge&&/ahorra/i.test(p.badge));
