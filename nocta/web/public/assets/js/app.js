...
window.nCardM=(p,o={})=>{const ph=NOCTA_PHOTOS[p.slug]||{};const lim=ph.limpio;const src=lim||ph.main||nImg(p);const cut=lim?true:(!ph.main&&nIsCut(src));const w=CATW[p.slug]||nCat(p).toLowerCase();
  return`<article class="n-pm n-reveal" data-slug="${p.slug}"><a class="n-pm__w" href="/producto.html?p=${p.slug}" aria-label="${p.name}">${p.compare?`<span class="n-pm__b">−${Math.round((1-p.price/p.compare)*100)} %</span>`:(p.badge?`<span class="n-pm__b n-pm__b--v">${p.badge.toLowerCase()}</span>`:'')}<img src="${src}" class="${cut?'is-cut':''}" width="720" height="720" alt="" loading="${o.eager?'eager':'lazy'}"${o.eager?' fetchpriority="high"':''} decoding="async"></a><div class="n-pm__i"><b>${p.name}</b><span class="n-num">${eur(p.price)}${nPlanSuf(p)}${p.compare?` <s>${eur(p.compare)}</s>`:''}</span></div><button class="n-btn n-btn--fill n-pm__buy" data-buy="${p.slug}" data-src="${o.src||'mini'}">${p.plan?'Empezar':'Comprar'}</button>${nPay(p.slug,{src:o.src||'mini'})}</article>`;};
