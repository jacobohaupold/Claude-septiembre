...
window.nPlanBuilder=(el,p,o={})=>{const isW=p.plan.interval==='week';let st=nPlanCalc(p,o.opt);
  const draw=()=>{el.innerHTML=`<div class="n-plb"><span class="n-lab">${isW?'Zona de esta semana':'Zonas de parches · elige las que quieras'}</span><div class="n-plb__pills" role="group">${PZ.map(([z])=>`<button type="button" data-z="${z}" class="${st.zones.includes(z)?'is-on':''}" aria-pressed="${st.zones.includes(z)}">${z}</button>`).join('')}</div>${isW?'':`<span class="n-lab">Skincare para poros cada mes</span><div class="n-plb__pills" role="radiogroup">${PS.map(([k])=>`<button type="button" data-s="${k}" class="${st.skin===k?'is-on':''}" aria-pressed="${st.skin===k}">${k}</button>`).join('')}<button type="button" data-s="" class="${st.skin?'':'is-on'}" aria-pressed="${!st.skin}">Sin skincare</button></div>`}</div>`;
